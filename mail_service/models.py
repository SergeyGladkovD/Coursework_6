...
class Customer(models.Model):
    ...
    def __str__(self):
        return (
            f"Клиент сервиса {self.name} почта {self.email} комментарий {self.comment}"
        )
...
class Mailing(models.Model):
    ...
    start_date = models.DateTimeField(
        default=timezone.now, verbose_name="время старта рассылки"
    )
...
class MailingAttempt(models.Model):
    ...
    date_time = models.DateTimeField(
        verbose_name="дата и время последней отправки попытки", auto_now_add=True
    )
    status = models.BooleanField(verbose_name="статус попытки")
    response_server = models.CharField(
